...
# -*- coding: utf-8 -*-
"""
flip_with_model_v2.py

Train a small model, flip ONLY high-confidence disagreements from your base submission,
with:
  - asymmetric margins for 0->1 vs 1->0
  - per-query flip cap
  - flip budget sweep (e.g., 60/80/100/120/140)

Meter: Leaderboard accuracy (we generate candidates; you submit and report).
...
def select_with_caps(
    ...
    """
    Greedy pick by descending margin, but don't exceed `max_per_query` flips per query.
    """
    order = want_idx[np.argsort(-margins[want_idx])]
...
def main():
    ...
    ap.add_argument("--out_prefix", default="flipped_v2")
    ap.add_argument("--budgets", type=int, nargs="+", default=[60,80,100,120,140])
    ap.add_argument("--min_margin_up", type=float, default=0.055, help="min margin for 0->1 flips")
    ap.add_argument("--min_margin_down", type=float, default=0.070, help="min margin for 1->0 flips")
    ...
    prob = clf.predict_proba(X_te[feats].values)[:,1]

    # Margins (confidence to flip)
    # up (0->1): margin_up = prob;  down (1->0): margin_down = 1 - prob
    margin_up = prob
    margin_dn = 1.0 - prob

    # Candidates that actually want to flip AND pass min-margin thresholds
    want_up = np.where((base_y==0) & (margin_up >= args.min_margin_up) & (prob > 0.5))[0]
    want_dn = np.where((base_y==1) & (margin_dn >= args.min_margin_down) & (prob < 0.5))[0]

    log(f"[INFO] Candidates 0->1: {len(want_up)}  (min_margin_up={args.min_margin_up})")
    log(f"[INFO] Candidates 1->0: {len(want_dn)}  (min_margin_down={args.min_margin_down})")

    reports = []
    for B in args.budgets:
        # split budget between up/down roughly by their availability
        total_cand = len(want_up) + len(want_dn)
        if total_cand == 0:
            chosen = np.array([], dtype=int)
        else:
            frac_up = len(want_up) / total_cand
            bud_up  = max(0, int(round(B * frac_up)))
            bud_dn  = max(0, B - bud_up)

            pick_up = select_with_caps(want_up, margin_up, qids, args.max_per_query, bud_up)
            # For down flips, we should not count queries already used by up flips
            used_queries = set(qids[i] for i in pick_up)
            # Mask want_dn to deprioritize those queries where we've already flipped one
            mask_dn = np.array([qids[i] not in used_queries for i in want_dn])
            want_dn_filtered = want_dn[mask_dn]
            pick_dn = select_with_caps(want_dn_filtered, margin_dn, qids, args.max_per_query, bud_dn)
        ...
        reports.append({
            ...
            "first_100_indexes_in_base_order": [int(i) for i in chosen[:100]],
            "first_100_ids": base["id"].iloc[chosen[:100]].tolist()
        })
